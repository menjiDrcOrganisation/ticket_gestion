                ...
                <td class="px-6 py-4 text-center">
                  <div class="flex justify-center gap-2">
                    ...
                    <button onclick="openModal('detailsModal{{ $billet['id'] }}')" 
                            class="w-8 h-8 bg-gray-100 text-gray-700 rounded-full flex items-center justify-center">
...
<div id="resendModal{{ $billet['id'] }}" class="hidden fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
  <div class="bg-white p-6 rounded-xl max-w-md w-full">
    ...
    <div class="flex justify-end gap-3 mt-6">
      <button onclick="closeModal('resendModal{{ $billet['id'] }}')" class="px-4 py-2 bg-gray-100 rounded-lg">Annuler</button>
      <button class="px-4 py-2 bg-blue-600 text-white rounded-lg">Confirmer</button>
    </div>
...
<div id="detailsModal{{ $billet['id'] }}" class="hidden fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
  <div class="bg-white p-6 rounded-xl max-w-md w-full">
    ...
    <div class="space-y-3 text-sm">
      <div class="flex justify-between"><span>Client :</span> <span>{{ $billet['auteur'] }}</span></div>
      <div class="flex justify-between"><span>Type :</span> <span>{{ $billet['type'] }}</span></div>
      <div class="flex justify-between"><span>Quantité :</span> <span>{{ $billet['quantite'] }}</span></div>
      <div class="flex justify-between"><span>Total :</span> <span>{{ $billet['total'] }} {{ $billet['devise'] }}</span></div>

      <div class="flex flex-col items-center mt-3">
        <div id="qrcode-{{ $billet['id'] }}" class="border p-2 rounded-md bg-white"></div>

        <button onclick="downloadQRCode('{{ $billet['id'] }}')" 
                class="mt-4 px-3 py-1.5 bg-blue-600 text-white rounded-lg">
          Télécharger
        </button>
      </div>

      <div class="flex justify-between mt-3">
        <span>Date :</span> <span>{{ $billet['date'] }}</span>
      </div>

    </div>

    <div class="flex justify-end mt-6">
      <button onclick="closeModal('detailsModal{{ $billet['id'] }}')" class="px-4 py-2 bg-gray-100 rounded-lg">Fermer</button>
    </div>
