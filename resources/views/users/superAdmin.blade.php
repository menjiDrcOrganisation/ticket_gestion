                                ...
                                @forelse($users as $index => $user)
                                    <tr class="border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-center flex justify-center gap-2">
                                            <button onclick="openModal('updateModal{{ $user->id }}')" class="text-blue-600 hover:underline">Éditer</button>
                                            <button onclick="openModal('deleteModal{{ $user->id }}')" class="text-red-600 hover:underline">Supprimer</button>
                                        </td>
                                    </tr>
                                    ...
                                    <div id="updateModal{{ $user->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                                        <div class="bg-white rounded-lg shadow-lg w-96 p-6">
                                            ...
                                            <form action="{{ route('users.update', $user->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <x-app-input type="text" name="name" :value="$user->name" placeholder="Nom" />
                                                <x-app-input type="email" name="email" :value="$user->email" placeholder="Email" />
                                                <x-app-input type="text" name="role" :value="$user->role" placeholder="Rôle (ex: admin)" />
...
<div id="storeModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="bg-white rounded-lg shadow-lg w-96 p-6">
        ...
        <form action="{{ route('users.store') }}" method="POST">
            ...
            <x-app-input type="text" name="role" placeholder="Rôle (ex: admin)" />
